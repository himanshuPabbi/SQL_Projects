...
select car_bike_data.*,
avg(Kms_Driven) over(partition by Seller_Type,Fuel_Type,Year order by Kms_Driven desc) as Avg_Kms_Driven,
sum(Kms_Driven) over(partition by Seller_Type,Fuel_Type,Year order by Kms_Driven desc) as Sum_Kms_Driven,
min(Kms_Driven) over(partition by Seller_Type,Fuel_Type,Year order by Kms_Driven desc
RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) as Min_Kms_Driven,
max(Kms_Driven) over(partition by Seller_Type,Fuel_Type,Year order by Kms_Driven desc) as Max_Kms_Driven
from car_bike_data;
...
#Per year find avg/min/max/sum Kms_Driven for CNG and Diesel
select  
avg(Kms_Driven) as AVG_Kms_Driven,
min(Kms_Driven) as Min_Kms_Driven,
max(Kms_Driven) as Max_Kms_Driven,
sum(Kms_Driven) as Total_Kms_Driven,
Fuel_Type,Year
from car_bike_data group by Fuel_Type,Year;

#per year which car has maximum and minimum Kms_Driven 
select car_bike_data.*,
max(Kms_Driven) over(partition by Year) as MAX_Kms_Driven,
min(Kms_Driven) over(partition by Year) as MIN_Kms_Driven
from car_bike_data;

#count how many CNG and Diesel cars with manual and automatic transmission with seller type Dealer and Individual 
select car_bike_data.*,count(*) over(partition by Fuel_Type,Transmission,Seller_Type) as CountNumbers
from car_bike_data;

#count how many CNG and Diesel cars with manual and automatic transmission  
select car_bike_data.*,count(*) over(partition by Fuel_Type,Transmission) as CountNumbers
from car_bike_data;

#Find Max/Min/Avg/sum kms driven for each seller type and fuel type
select car_bike_data.*,
max(Kms_Driven) over(partition by Seller_Type,Fuel_Type order by Kms_Driven desc) as Max_Kms_Driven,
min(Kms_Driven) over(partition by Seller_Type,Fuel_Type order by Kms_Driven desc
RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) as Min_Kms_Driven,
avg(Kms_Driven) over(partition by Seller_Type,Fuel_Type order by Kms_Driven desc) as Avg_Kms_Driven,
sum(Kms_Driven) over(partition by Seller_Type,Fuel_Type order by Kms_Driven desc) as Sum_Kms_Driven
from car_bike_data;

#Find Max/Min/Avg/sum kms driven for each seller type and fuel type and Transmission
select car_bike_data.*,
max(Kms_Driven) over(partition by Seller_Type,Fuel_Type,Transmission order by Kms_Driven desc) as Max_Kms_Driven,
min(Kms_Driven) over(partition by Seller_Type,Fuel_Type,Transmission order by Kms_Driven desc
RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) as Min_Kms_Driven,
avg(Kms_Driven) over(partition by Seller_Type,Fuel_Type,Transmission order by Kms_Driven desc) as Avg_Kms_Driven,
sum(Kms_Driven) over(partition by Seller_Type,Fuel_Type,Transmission order by Kms_Driven desc) as Sum_Kms_Driven
from car_bike_data;

#Find Max/Min/Avg present price for each seller type,fuel type and Transmission
select car_bike_data.*,
max(Present_Price) over(partition by Seller_Type,Fuel_Type,Transmission order by Present_Price desc)  as Max_PresentPrice,
min(Present_Price) over(partition by Seller_Type,Fuel_Type,Transmission order by Present_Price desc
range between unbounded preceding and unbounded following)  as Min_PresentPrice,
round(avg(Present_Price) over(partition by Seller_Type,Fuel_Type,Transmission order by Present_Price desc
range between unbounded preceding and unbounded following),2)  as Avg_PresentPrice
from car_bike_data;